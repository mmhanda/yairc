...
/*   Updated: 2023/03/07 12:21:20 by atabiti          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
*/
	while (1)
	{
		...
		if (!input.empty())
		{
            /*const_cast <new_type> (expression) why? strtok uses char * and input.c_str() return const char* */
			char *str = const_cast<char *>(input.c_str());
			str = strtok(str, " ");
            if(str != NULL)
        {
    	    std::cout << "i am here no segfault "  << std::endl;
			while (str != NULL)
			{
